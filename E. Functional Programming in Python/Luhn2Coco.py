...
@_coconut_tco
def toDigits(n  # type: Union[str, int]
    ):
# type: (...) -> [int]
    _coconut_match_to = n
    ...
    if _coconut_match_check:
        return _coconut_tail_call(int, n)
    else:
        return [toDigits(d) for d in str(n)]

@_coconut_tco
def doubleEveryOther(n  # type: [int]
    ):
# type: (...) -> [int]
    return _coconut_tail_call((list), map(_coconut.operator.mul, *(n, cycle([1, 2]))))
# return [(a*b) for (a,b) in zip(n, cycle([1,2]))]

@_coconut_tco
def sumDigits(n  # type: [int]
    ):
# type: (...) -> int
    return _coconut_tail_call(reduce, _coconut.operator.add, n)

@_coconut_tco
def checkSum(n  # type: int
    ):
# type: (...) -> int
    return _coconut_tail_call((sumDigits), (doubleEveryOther)(toDigits(n)))

def isValid(n  # type: int
    ):
    return checkSum(n) % 10 == 0

def testCC():
# type: (...) -> [bool]
    (print)((list)(map(isValid, [1234567890123456, 1234567890123452])))
